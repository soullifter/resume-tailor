...
export default function ModelWidget() {
  ...
  return (
    <div ref={ref} className="relative flex items-center gap-1.5">
      {/* Rate info */}
      <span className={`text-xs hidden sm:block ${pctColor(tokRemain, tokTotal, exhausted)}`}>
      ...
      <button
        onClick={() => setOpen(v => !v)}
        className={`flex items-center gap-1 text-xs font-medium border rounded-lg px-2 py-1 transition-colors ${
          exhausted
      ...
      {open && (
        <div className="fixed inset-x-3 top-16 z-50 sm:absolute sm:inset-x-auto sm:top-8 sm:right-0 sm:w-72 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
          <div className="px-3 py-2 border-b border-slate-800 flex items-center justify-between">
            <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Switch AI Model</p>
            <span className={`text-xs sm:hidden ${pctColor(tokRemain, tokTotal, exhausted)}`}>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    {isActive ? <span className="text-blue-400 text-xs shrink-0">✓</span> : <span className="text-xs text-slate-600 shrink-0">#{idx + 1}</span>}
                  ...
                  <div className="flex items-center gap-2 mt-1 ml-5">
                    {exhausted
                      ? <span className="text-xs px-1.5 py-0.5 rounded-full border font-medium text-red-400 bg-red-500/10 border-red-500/20">Limit reached</span>
                      : opt.tag && <span className={`text-xs px-1.5 py-0.5 rounded-full border font-medium ${opt.tagColor}`}>{opt.tag}</span>
                    }
                    <span className={`text-xs tabular-nums ${pctColor(optRemain, optTotal, exhausted)}`}>
                      {exhausted ? 'Limit reached' : optDefault ? '~100%' : fmtPct(optRemain, optTotal)}
                    </span>
                  </div>
                  {/* Row 3: description */}
                  <p className="text-xs text-slate-500 mt-1 ml-5 leading-snug">{opt.desc}</p>
                </button>
              )
            })}
          </div>
